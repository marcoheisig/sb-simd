...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (sb-simd::macro-when (and +avx2+ +fma+)
  ...
  (sb-simd::macro-when +avx+
    ...
    (defknown (%f64.2-rec13) ((simd-pack double-float)
                              (simd-pack double-float))
        (simd-pack double-float)
        (movable flushable always-translatable)
        :overwrite-fndb-silently t)
    (define-vop (%f64.2-rec13)
      (:translate %f64.2-rec13)
      (:policy :fast-safe)
      (:args (x :scs (double-sse-reg))
    ...
    (defknown (%f64.2-rec9) ((simd-pack double-float)
                             (simd-pack double-float))
        (simd-pack double-float)
        (movable flushable always-translatable)
      :overwrite-fndb-silently t)
    (define-vop (%f64.2-rec9)
      (:translate %f64.2-rec9)
      (:policy :fast-safe)
      (:args (x :scs (double-sse-reg))
    ...
    (defknown (%f64.4-rsqrt9) ((simd-pack-256 double-float)
                               (simd-pack-256 double-float)
                               (simd-pack-256 double-float)
                               (simd-pack-256 double-float))
        (simd-pack-256 double-float)
    ...
    (defknown (%f64.2-rsqrt13) ((simd-pack double-float)
                                (simd-pack double-float)
                                (simd-pack double-float))
        (simd-pack double-float)
        (movable flushable always-translatable)
      :overwrite-fndb-silently t)
    (define-vop (%f64.2-rsqrt13)
      (:translate %f64.2-rsqrt13)
      (:policy :fast-safe)
      (:args (x :scs (double-sse-reg))
    ...
    (defknown (%f64.2-rsqrt9) ((simd-pack double-float)
                               (simd-pack double-float)
                               (simd-pack double-float)
                               (simd-pack double-float))
        (simd-pack double-float)
        (movable flushable always-translatable)
      :overwrite-fndb-silently t)
    (define-vop (%f64.2-rsqrt9)
      (:translate %f64.2-rsqrt9)
      (:policy :fast-safe)
      (:args (x :scs (double-sse-reg))
    ...
    (defknown (%f64.4-blend)
        ...
        (flushable)
      :overwrite-fndb-silently t)
    (define-vop (%f64.4-blend)
      (:translate %f64.4-blend)
      (:args (x :scs (double-avx2-reg))
    ...
    (defknown (%f64.4-permute)
        ((simd-pack-256 double-float)
         (integer 0 15))
        (simd-pack-256 double-float)
        (flushable)
      :overwrite-fndb-silently t)
    (define-vop (%f64.4-permute)
      (:translate %f64.4-permute)
      (:args (x :scs (double-avx2-reg)))
      (:arg-types simd-pack-256-double
                  (:constant (integer 0 15)))
      (:info mask)
    ...
    (defknown (%f64.4-permute2f128)
        ...
        (flushable)
      :overwrite-fndb-silently t)
    (define-vop (%f64.4-permute2f128)
      (:translate %f64.4-permute2f128)
      (:args (x :scs (double-avx2-reg))
             (y :scs (double-avx2-reg)))
      (:arg-types simd-pack-256-double
                  simd-pack-256-double
                  (:constant (integer 0 3)))
      (:info mask)
      (:results (dst :scs (double-avx2-reg)))
      (:result-types simd-pack-256-double)
      (:policy :fast-safe)
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (macro-when +avx+
    ...
    (define-inline f64.4-rec13 (%x)
      (declare (optimize speed))
      (sb-vm::%f64.4-rec13 %x (f64.4-broadcast 2)))
    (export 'f64.4-rec13)

    (declaim (ftype (function (f64.4) f64.4) f64.4-rec9))
    (define-inline f64.4-rec9 (%x)
      (declare (optimize speed))
      (sb-vm::%f64.4-rec9 %x (f64.4-broadcast 3)))
    (export 'f64.4-rec9)

    (declaim (ftype (function (f64.2) f64.2) f64.2-rec13))
    (define-inline f64.2-rec13 (%x)
      (declare (optimize speed))
      (sb-vm::%f64.2-rec13 %x (f64.2-broadcast 2)))
    (export 'f64.2-rec13)
    
    (declaim (ftype (function (f64.2) f64.2) f64.2-rec9))
    (define-inline f64.2-rec9 (%x)
      (declare (optimize speed))
      (sb-vm::%f64.2-rec9 %x (f64.2-broadcast 3)))
    (export 'f64.2-rec9)

    (declaim (ftype (function (f64.4) f64.4) f64.4-rsqrt13))
    (define-inline f64.4-rsqrt13 (%x)
      (declare (optimize speed))
      (sb-vm::%f64.4-rsqrt13 %x (f64.4-broadcast 0.5)
                                (f64.4-broadcast 1.5)))
    ...
    (define-inline f64.4-rsqrt9 (%x)
      (declare (optimize speed))
      (sb-vm::%f64.4-rsqrt9 %x (f64.4-broadcast 0.375)
                               (f64.4-broadcast 1.250)
                               (f64.4-broadcast -1.875)))
    (export 'f64.4-rsqrt9)

    (declaim (ftype (function (f64.2) f64.2) f64.2-rsqrt13))
    (define-inline f64.2-rsqrt13 (%x)
      (declare (optimize speed))
      (sb-vm::%f64.2-rsqrt13 %x (f64.2-broadcast 0.5)
                                (f64.2-broadcast 1.5)))
    (export 'f64.2-rsqrt13)

    (declaim (ftype (function (f64.2) f64.2) f64.2-rsqrt9))
    (define-inline f64.2-rsqrt9 (%x)
      (declare (optimize speed))
      (sb-vm::%f64.2-rsqrt9 %x (f64.2-broadcast 0.375)
                               (f64.2-broadcast 1.250)
                               (f64.2-broadcast -1.875)))
    (export 'f64.2-rsqrt9)
    ...
    (define-inline f64.4-extractf128 (%x index)
      (declare (optimize speed))
      (case index
        (0 (sb-vm::%f64.4-extractf128 %x 0))
        (1 (sb-vm::%f64.4-extractf128 %x 1))))
    ...
    (define-inline f32.8-extractf128 (%x index)
      (declare (optimize speed))
      (case index
        (0 (sb-vm::%f32.8-extractf128 %x 0))
        (1 (sb-vm::%f32.8-extractf128 %x 1))))
    ...
    (define-inline f64.4-blend (%x %y mask)
      (declare (optimize speed))
      (case mask
          (#b0000 (sb-vm::%f64.4-blend %x %y #b0000))
          (#b0001 (sb-vm::%f64.4-blend %x %y #b1000))
          (#b0010 (sb-vm::%f64.4-blend %x %y #b0100))
          (#b0100 (sb-vm::%f64.4-blend %x %y #b0010))
          (#b1000 (sb-vm::%f64.4-blend %x %y #b0001))
          (#b1001 (sb-vm::%f64.4-blend %x %y #b1001))
          (#b1010 (sb-vm::%f64.4-blend %x %y #b0101))
          (#b1100 (sb-vm::%f64.4-blend %x %y #b0011))
          (#b1101 (sb-vm::%f64.4-blend %x %y #b1011))
          (#b1110 (sb-vm::%f64.4-blend %x %y #b0111))
          (#b1111 (sb-vm::%f64.4-blend %x %y #b1111))
          (#b0111 (sb-vm::%f64.4-blend %x %y #b1110))
          (#b0011 (sb-vm::%f64.4-blend %x %y #b1101))
          (#b1011 (sb-vm::%f64.4-blend %x %y #b1110))
          (#b0101 (sb-vm::%f64.4-blend %x %y #b1010))
          (t (sb-vm::%f64.4-blend %x %y #b0110))))
    (export 'f64.4-blend)

    (declaim (ftype (function (f64.4 (integer 0 15)) f64.4) f64.4-permute)) 
    (define-inline f64.4-permute (%x mask)
      (declare (optimize speed))
      (case mask
        (#b0000 (sb-vm::%f64.4-permute %x #b0000))
        (#b0001 (sb-vm::%f64.4-permute %x #b1000))
        (#b0010 (sb-vm::%f64.4-permute %x #b0100))
        (#b0100 (sb-vm::%f64.4-permute %x #b0010))
        (#b1000 (sb-vm::%f64.4-permute %x #b0001))
        (#b1001 (sb-vm::%f64.4-permute %x #b1001))
        (#b1010 (sb-vm::%f64.4-permute %x #b0101))
        (#b1100 (sb-vm::%f64.4-permute %x #b0011))
        (#b1101 (sb-vm::%f64.4-permute %x #b1011))
        (#b1110 (sb-vm::%f64.4-permute %x #b0111))
        (#b1111 (sb-vm::%f64.4-permute %x #b1111))
        (#b0111 (sb-vm::%f64.4-permute %x #b1110))
        (#b0011 (sb-vm::%f64.4-permute %x #b1101))
        (#b1011 (sb-vm::%f64.4-permute %x #b1110))
        (#b0101 (sb-vm::%f64.4-permute %x #b1010))
        (t (sb-vm::%f64.4-permute %x #b0110))))
    (export 'f64.4-permute)

    (declaim (ftype (function (f64.4 f64.4 (integer 0 3)) f64.4) f64.4-permute2f128)) 
    (define-inline f64.4-permute2f128 (%x %y mask)
      (declare (optimize speed))
      (case mask
          (#b00 (sb-vm::%f64.4-permute2f128 %x %y #b00))
          (#b01 (sb-vm::%f64.4-permute2f128 %x %y #b10))
          (#b10 (sb-vm::%f64.4-permute2f128 %x %y #b01))
          (t (sb-vm::%f64.4-permute2f128 %x %y #b11))))
    ...
    (define-inline f64.4-vdot (u v)
      ...
      (let* ((n  (min (array-total-size u) (array-total-size v)))
        ...
        (if (< n 4)
            ...
            (+ (loop with %sum of-type f64.4 = (f64.4-broadcast 0)
                     for i of-type fixnum below n0 by 4
                     do (f64.4-incf %sum (f64.4* (f64.4-aref u i) (f64.4-aref v i)))
    ...
    (define-inline f32.8-vdot (u v)
      ...
      (let* ((n  (min (array-total-size u) (array-total-size v)))
        ...
        (if (< n 8)
            ...
            (+ (loop with %sum of-type f32.8 = (f32.8-broadcast 0)
                     for i of-type fixnum below n0 by 8
                     do (f32.8-incf %sum (f32.8* (f32.8-aref u i) (f32.8-aref v i)))
  ...
  (sb-simd::macro-when +avx2+
    ...
    (define-inline u32.8-extracti128 (%x index)
      ...
      (case index
        (0 (sb-vm::%u64.4-extracti128 %x index))
        (1 (sb-vm::%u64.4-extracti128 %x index))))
    (export 'u32.8-extracti128)) ;; end of +avx2+ 
    ...
    (let* ((n  (min (array-total-size u) (array-total-size v)))
      ...
      (if (< n 2)
          ...
          (+ (loop with %sum of-type f64.2 = (make-f64.2 0 0)
                   ...
                   do (f64.2-incf %sum (f64.2* (f64.2-aref u i)
                                               (f64.2-aref v i)))
    ...
    (let* ((n  (min (array-total-size u) (array-total-size v)))
      ...
      (if (< n 4)
          ...
          (+ (loop with %sum of-type f64.4 = (make-f64.4 0 0 0 0)
                   for i of-type fixnum below n0 by 4
                   do (f64.4-incf %sum (f64.4* (f64.4-aref u i)
                                               (f64.4-aref v i)))
                   finally (return (f64.4-hsum %sum)))
             (loop for i of-type fixnum from n0 below n
                   summing (* (aref u i) (aref v i))
    ...
    (let* ((n  (min (array-total-size u) (array-total-size v)))
      ...
      (if (< n 4)
          ...
          (+ (loop with %sum of-type f32.4 = (make-f32.4 0 0 0 0)
                   for i of-type fixnum below n0 by 4
                   do (f32.4-incf %sum (f32.4* (f32.4-aref u i)
                                               (f32.4-aref v i)))
  ...
  (define-inline f64.2-vsum (u)
    ...
    (let* ((n  (array-total-size u))
      ...
      (if (< n 2)
          ...
          (+ (loop with %sum of-type f64.2 = (make-f64.2 0 0)
                   for i of-type fixnum below n0 by 2
                   do (f64.2-incf %sum (f64.2-aref u i))
                   finally (return (f64.2-hsum %sum)))
             (loop for i of-type fixnum from n0 below n
                   summing (row-major-aref u i)
                     into sum of-type double-float
  ...
  (define-inline f32.4-vsum (u)
    ...
    (let* ((n  (array-total-size u))
      ...
      (if (< n 4)
          ...
          (+ (loop with %sum of-type f32.4 = (make-f32.4 0 0 0 0)
                   for i of-type fixnum below n0 by 4
                   do (f32.4-incf %sum (f32.4-aref u i))
                   finally (return (f32.4-hsum %sum)))
             (loop for i of-type fixnum from n0 below n
                   summing (row-major-aref u i)
                     into sum of-type single-float
  ...
  (sb-simd::macro-when (and +avx2+ +fma+)
    ...
    (export 'f32.8-vdot)) ;; end of (and +avx2+ +fma+)
  ) ;;end of eval-when
