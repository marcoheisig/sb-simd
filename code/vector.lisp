...
(declaim (ftype (function (f64vec) f64) f64.2-vsum)
         (inline f64.2-vsum))
(defun f64.2-vsum (array &aux (n (array-total-size array)))
  (declare (optimize speed (safety 0)))
  (do ((index 0 (the (integer 0 #.(- array-total-size-limit 8)) (+ index 8)))
       (acc1 (make-f64.2 0 0) (f64.2-incf acc1 (f64.2-aref array (+ index 0))))
       (acc2 (make-f64.2 0 0) (f64.2-incf acc2 (f64.2-aref array (+ index 4))))
       (acc3 (make-f64.2 0 0) (f64.2-incf acc3 (f64.2-aref array (+ index 8))))
       (acc4 (make-f64.2 0 0) (f64.2-incf acc4 (f64.2-aref array (+ index 12)))))
      ((>= index (- n 8))
       (do ((result (multiple-value-call #'+ (f64.2-values (f64.2+ acc1 acc2 acc3 acc4)))
...
(declaim (ftype (function (f32vec) f32) f32.4-vsum)
         (inline f32.4-vsum))
(defun f32.4-vsum (array &aux (n (array-total-size array)))
  (declare (optimize speed (safety 0)))
  (do ((index 0 (the (integer 0 #.(- array-total-size-limit 16)) (+ index 16)))
       (acc1 (make-f32.4 0 0 0 0) (f32.4-incf acc1 (f32.4-aref array (+ index 0))))
       (acc2 (make-f32.4 0 0 0 0) (f32.4-incf acc2 (f32.4-aref array (+ index 4))))
       (acc3 (make-f32.4 0 0 0 0) (f32.4-incf acc3 (f32.4-aref array (+ index 8))))
       (acc4 (make-f32.4 0 0 0 0) (f32.4-incf acc4 (f32.4-aref array (+ index 12)))))
      ((>= index (- n 16))
       (do ((result (multiple-value-call #'+ (f32.4-values (f32.4+ acc1 acc2 acc3 acc4)))
                    (+ result (row-major-aref array index)))
            (index index (1+ index)))
