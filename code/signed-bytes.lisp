...
(progn
  ...
  (defun %make-simd-pack-sb64 (p0 p1)
    ...
    (%make-simd-pack-ub64 (ldb (byte 64 0) p0) (ldb (byte 64 0) p1)))
  (export '%make-simd-pack-sb64)
  ...
  (defun %make-simd-pack-sb32 (p0 p1 p2 p3)
    ...
    (%make-simd-pack-ub64
     (logior (ldb (byte 32 0) p0) (ash (ldb (byte 32 0) p1) 32))
     (logior (ldb (byte 32 0) p2) (ash (ldb (byte 32 0) p3) 32))))
  (export '%make-simd-pack-sb32)
  ...
  (defun %make-simd-pack-sb8 (p0 p1 p2 p3 p4 p5 p6 p7
    ...
    (declare (type (signed-byte 8) p0 p1 p2 p3 p4 p5 p6 p7
                   p8 p9 p10 p11 p12 p13 p14 p15))
    (%make-simd-pack-ub16
     (logior (ldb (byte 8 0) p0)  (ash (ldb (byte 8 0) p1)  8))
...
(progn
  ...
  (defun %simd-pack-256-sb64s (pack)
    ...
    (values (sb-c::mask-signed-field 64 (sb-vm::%simd-pack-256-0 pack))
            (sb-c::mask-signed-field 64 (sb-vm::%simd-pack-256-1 pack))
  ...
  (defun %make-simd-pack-256-sb32 (p0 p1 p2 p3 p4 p5 p6 p7)
    ...
    (%make-simd-pack-256-ub64
     (logior (ldb (byte 32 0) p0) (ash (ldb (byte 32 0) p1) 32))
     (logior (ldb (byte 32 0) p2) (ash (ldb (byte 32 0) p3) 32))
     (logior (ldb (byte 32 0) p4) (ash (ldb (byte 32 0) p5) 32))
     (logior (ldb (byte 32 0) p6) (ash (ldb (byte 32 0) p7) 32))))
  (export '%make-simd-pack-256-sb32)

  (defun %simd-pack-256-sb32s (pack)
    (declare (type simd-pack-256 pack))
    (let ((p0 (sb-vm::%simd-pack-256-0 pack))
          (p1 (sb-vm::%simd-pack-256-1 pack))
          (p2 (sb-vm::%simd-pack-256-2 pack))
          (p3 (sb-vm::%simd-pack-256-3 pack)))
  ...
  (export' %simd-pack-256-sb32s)

  (defun %make-simd-pack-256-sb16 (p0 p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12 p13
                                   p14 p15)
    (declare (type (signed-byte 16) p0 p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12
                                    p13 p14 p15))
  ...
  (export '%simd-pack-256-sb16s)

  (defun %make-simd-pack-256-sb8 (p0 p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12 p13
                                  p14 p15 p16 p17 p18 p19 p20 p21 p22 p23 p24
                                  p25 p26 p27 p28 p29 p30 p31)
    (declare (type (signed-byte 8) p0 p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12
                   p13 p14 p15 p15 p17 p18 p19 p20 p21 p22 p23 p24 p25 p26 p27
                   p28 p29 p30 p31))
